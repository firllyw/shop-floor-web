...
export default function ToolManagement() {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", requiresCheckout: false });
  ...
  const handleOpenModal = (tool?: any) => {
    if (tool) {
      setEditingTool(tool);
      setFormData({ name: tool.name, description: tool.description || "", requiresCheckout: tool.requiresCheckout });
    } else {
      setEditingTool(null);
      setFormData({ name: "", description: "", requiresCheckout: false });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-gray-500">Manage manufacturing tools and checkout requirements.</p>
        </div>
        <button
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-gray-600 border-b">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Requires Checkout</th>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center mt-2">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, requiresCheckout: e.target.checked })}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="requiresCheckout" className="ml-2 block text-sm text-gray-900">
                  Requires Checkout (for expensive tools)
                </label>
              </div>
